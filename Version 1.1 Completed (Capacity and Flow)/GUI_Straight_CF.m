...
function [segmentl_t1, segmentr_t1] = GUI_Straight_CF(segmentl_t, segmentr_t, mlt_C, mrt_C, mlt_F, mrt_F, l)
...
%% Calculations
    ...
    Rl = max(min([mlt_F,(w/v)*(mlt_C-segmentl_t)],[],2),0); % Calculate the recieve capacity for each cell
    ...
    Rr = max(min([mrt_F,(w/v)*(mrt_C-segmentr_t)],[],2),0); % Calculate the recieve capacity for each cell
