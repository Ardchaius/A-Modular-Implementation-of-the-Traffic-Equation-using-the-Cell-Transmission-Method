...
if (0 < timer) && (timer < light_Switch + light_Pause + 1) % Up down lanes have green light
    ...
    [mr_1,ml_1,slr_1,sll_1,srr_1,srl_1,topc_1,botc_1] = GUI_Traffic_Light_Sub_CF...
        ...
        mrsltr,mrsrtr,mlsltr,mlsrtr,ls);
...
function [mr_1,ml_1,slr_1,sll_1,srr_1,srl_1,topc_1,botc_1] = GUI_Traffic_Light_Sub_CF...
    ...
    splitVeh = SplitFunc(ml(ls+2),mlsltr);
    slr = [splitVeh(1);slr];
    slr_C = [ml_C(ls+2);slr_C];
    slr_F = [ml_F(ls+2);slr_F];
    ml(ls+2) = splitVeh(2);
    splitVeh = SplitFunc(mr(ls+2),mrsrtr);
    srr = [splitVeh(1);srr];
    srr_C = [mr_C(ls+2);srr_C];
    srr_F = [mr_F(ls+2);srr_F];
    mr(ls+2) = splitVeh(2);
    
    splitVeh = SplitFunc(mr(ls+4),mrsltr);
    topc = [splitVeh(1);topc;slr(2)];
    topc_C = [mr_C(ls+4);topc_C;slr_C(2)];
    topc_F = [mr_F(ls+4);topc_F;slr_F(2)];
    mr(ls+4) = splitVeh(2);
    splitVeh = SplitFunc(ml(ls+4),mlsrtr);
    botc = [splitVeh(1);botc;srr(2)];
    ...
    Rml = max(min([ml_F,(ml_C-ml)],[],2),0); % Calculate the recieve capacity for each cell left lane of main road
    Smr = min([mr_F,mr],[],2); % Calculate the send capacity for each cell right lane of main road
    Rmr = max(min([mr_F,(mr_C-mr)],[],2),0); % Calculate the recieve capacity for each cell right lane of main road
    
    Ssll = min([sll_F,sll],[],2); % Calculate send capacity for the left lane of the left side road
    Rsll = max(min([sll_F,(sll_C-sll)],[],2),0); % Calculate recieve capacity for the left lane of the left side road
    Ssrl = min([srl_F,srl],[],2); % Calculate send capacity for the left lane of the right side road
    Rsrl = max(min([srl_F,(srl_C-srl)],[],2),0); % Calculate recieve capacity for the left lane of the right side road
    
    Sslr = min([slr_F,slr],[],2); % Calculate send capacity for the right lane of the left side road
    Rslr = max(min([slr_F,(slr_C-slr)],[],2),0); % Calculate recieve capacity for the right lane of the left side road
    Ssrr = min([srr_F,srr],[],2); % Calcualte send capacity for the right lane of the right side road
    Rsrr = max(min([srr_F,(srr_C-srr)],[],2),0); % Calcualte recieve capacity for the right lane of the right side road
    ...
    Rtopc = max(min([topc_F,(topc_C-topc)],[],2),0); % Calculate recieve capacity for cell between the main lanes top
    Sbotc = min([botc_F,botc],[],2); % Calculate send capacity for cell between the main lanes bottom
    Rbotc = max(min([botc_F,(botc_C-botc)],[],2),0); % Calculate recieve capacity for cell between the main lanes bottom
