...
% Last Modified by GUIDE v2.5 29-May-2019 02:26:58
...
if nargin<4 | ~isstruct(varargin{1})
    ...
    tempstring = sprintf('Auto Play %c',char(9654));
    ...
    handles.cMap = [1 1 1;
        (0:0.001:1)',(1:-0.001:0)',zeros(1001,1)];
...
if get(0,'UserData')
    tempstring = sprintf('Auto Play %c',char(9208));
...
Edit_Window(input,handles.Connections,handles.Segments,handles.InOut_T,hObject);
...
[handles.FileName, ~] = uiputfile('*.txt');
guidata(hObject,handles);

save_Network(handles);
...
for i = 1:simSize
    for j = 1:simSize
        ...
        temp = segments{i,j}; % Saving data matrix to temporary for alteration for passing to functions
        ...
        switch road_Network(i,j)
            ...
            case char(9472) % Straight L-R
                ...
                [mlt1,mrt1] = GUI_Straight(mlt,mrt,length);
                inOut_T1(i,j) = {[0,mrt1(end),0,mlt1(end);...
                    0,mlt1(1),0,mrt1(1)]};
            ...
            case char(9474) % Straight T-B
                ...
                [mlt1,mrt1] = GUI_Straight(mlt,mrt,length);
                inOut_T1(i,j) = {[mrt1(end),0,mlt1(end),0;...
                    mlt1(1),0,mrt1(1),0]};
                segments(i,j) = {[mlt1,mrt1]};
            case char(9488) % Corner L-B
                ...
                [mlt1,mrt1] = GUI_Straight(mlt,mrt,length-2);
                inOut_T1(i,j) = {[0,0,mrt1(end),mlt1(end);...
                    0,0,mlt1(1),mrt1(1)]};
                segments(i,j) = {[mlt1,mrt1]};
            case char(9484) % Corner B-R
                ...
                [mlt1,mrt1] = GUI_Straight(mlt,mrt,length-2);
                inOut_T1(i,j) = {[0,mrt1(end),mlt1(end),0;...
                    0,mlt1(1),mrt1(1),0]};
                segments(i,j) = {[mlt1,mrt1]};
            case char(9492) % Corner R-T
                ...
                [mlt1,mrt1] = GUI_Straight(mlt,mrt,length-2);
                inOut_T1(i,j) = {[mrt1(end),mlt1(end),0,0;...
                    mlt1(1),mrt1(1),0,0]};
                segments(i,j) = {[mlt1,mrt1]};
            case char(9496) % Corner T-L
                ...
                [mlt1,mrt1] = GUI_Straight(mlt,mrt,length-2);
                inOut_T1(i,j) = {[mlt1(end),0,0,mrt1(end);...
                    mrt1(1),0,0,mlt1(1)]};
                segments(i,j) = {[mlt1,mrt1]};
            case char(9508) % Intersection from Left
                temp_top = inOut_T{i-1,j};
                ...
                t2t = temp(end,1);
                t1t = temp(end,2);
                [mrt1,mlt1,srt1,slt1,t1t1,t2t1] = GUI_Intersection(mrt,mlt,srt,slt,t1t,t2t,length);
                inOut_T1(i,j) = {[mrt1(end),0,mlt1(end),srt1(end);...
            ...
            case char(9516) % Intersection from Bottom
                ...
                t1t = temp(end,2);
                [mrt1,mlt1,srt1,slt1,t1t1,t2t1] = GUI_Intersection(mrt,mlt,srt,slt,t1t,t2t,length);
                inOut_T1(i,j) = {[0,mlt1(end),srt1(end),mrt1(end);...
            ...
            case char(9500) % Intersection from Right
                ...
                t1t = temp(end,2);
                [mrt1,mlt1,srt1,slt1,t1t1,t2t1] = GUI_Intersection(mrt,mlt,srt,slt,t1t,t2t,length);
                inOut_T1(i,j) = {[mlt1(end),srt1(end),mrt1(end),0;...
            ...
            case char(9524) % Intersection from Top
                ...
                t1t = temp(end,2);
                [mrt1,mlt1,srt1,slt1,t1t1,t2t1] = GUI_Intersection(mrt,mlt,srt,slt,t1t,t2t,length);
                inOut_T1(i,j) = {[srt1(end),mrt1(end),0,mlt1(end);...
            ...
            case char(9711) % Roundabout
                ...
                [mtt1,mrt1,mbt1,mlt1,stlt1,strt1,srlt1,srrt1,sblt1,sbrt1,sllt1,slrt1]...
                    = GUI_Roundabout(mtt,mrt,mbt,mlt,stlt,strt,srlt,srrt,sblt,sbrt,sllt,slrt,length);
                inOut_T1(i,j) = {[strt1,srrt1,sbrt1,slrt1;...
            ...
            case char(9532) % 4-Way traffic light
                ...
                [tl_1,tr_1,rl_1,rr_1,bl_1,br_1,ll_1,lr_1,tc_1,rc_1,bc_1,lc_1,timer]...
                    = GUI_Traffic_Light(tl,tr,rl,rr,bl,br,ll,lr,tc,rc,bc,lc,length,timer);
                inOut_T1(i,j) = {[tr_1(end),rr_1(end),br_1(end),lr_1(end);...
...
for i = 1:simSize
    for j = 1:simSize
        ...
        visualization(1+(i-1)*length:i*length,1+(j-1)*length:j*length) = vis_Segment;
        
    end
end
heatmap(visualization,'Colormap',handles.cMap,'ColorLimits',[0,3.05],'GridVisible','off','CellLabelColor','none');
...
if strcmp(inputVolume,'random')
    fprintf(fileID,[inputVolume newline]);
else
    fprintf(fileID,'%i\n',inputVolume);
end
...
for i = 1:simSize
    ...
    for j = 1:simSize  
        ...
        temp_Segment = segments{i,j};
        temp_T = inOut_T{i,j};
        temp_T1 = inOut_T1{i,j};
        temp_Connections = connections{i,j};
        fprintf(fileID,'%i',char(road_Network(i,j)));
        fprintf(fileID,newline);
        fprintf(fileID,'%d ',temp_Segment(:));
        fprintf(fileID, newline);
        fprintf(fileID,'%d ',temp_T(:));
        fprintf(fileID, newline);
        fprintf(fileID,'%d ',temp_T1(:));
        fprintf(fileID, newline);
        fprintf(fileID,'%i ',temp_Connections(:));
        fprintf(fileID, newline);
...
handles.Size = str2double(fgetl(fileID));
segLength = str2double(fgetl(fileID));
handles.SegmentLength = segLength;
guidata(hObject,handles);

road_Network = strings(handles.Size);
