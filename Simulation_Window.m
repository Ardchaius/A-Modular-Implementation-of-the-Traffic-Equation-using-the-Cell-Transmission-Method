...
% Last Modified by GUIDE v2.5 14-Mar-2019 23:09:59
...
if nargin<4 | ~isstruct(varargin{1})
    ...
    handles.Connections = cell(simSize);
...
switch index
    ...
    case 14 % Input
        src.String = 'EX';
        connections(loci,locj) = {[1,1,1,1]};
    otherwise % For cancel or exit to avoid annoying ding sound from Matlab
...
handles = guidata(hObject);
valid = check_Validity(handles.Network,handles.Connections,handles.SimSize);
if valid
    input = struct('Network',handles.Network,'Size',handles.SimSize);
    close;
    Simulation_Plot(input,handles.Connections);
end
