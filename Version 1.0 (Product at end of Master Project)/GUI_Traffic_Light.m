...
function [mr_1,ml_1,slr_1,sll_1,srr_1,srl_1,topc_1,botc_1] = GUI_Traffic_Light_Sub(mr,ml,slr,sll,srr,srl,topc,botc,mrsltr,mrsrtr,mlsltr,mlsrtr,ls,v,w,nN,Nm,Ns,Nc,Qm,Qsl,Qsr,Qc)
    ...
    Ssll = min([Qsl,sll],[],2); % Calculate send capacity for the left lane of the left side road
    Rsll = min([Qsl,(w/v)*(Ns-sll)],[],2); % Calculate recieve capacity for the left lane of the left side road
    Ssrl = min([Qsl,srl],[],2); % Calculate send capacity for the left lane of the right side road
    Rsrl = min([Qsl,(w/v)*(Ns-srl)],[],2); % Calculate recieve capacity for the left lane of the right side road
    
    Sslr = min([Qsr,slr],[],2); % Calculate send capacity for the right lane of the left side road
    Rslr = min([Qsr,(w/v)*(Ns-slr)],[],2); % Calculate recieve capacity for the right lane of the left side road
    Ssrr = min([Qsr,srr],[],2); % Calcualte send capacity for the right lane of the right side road
    Rsrr = min([Qsr,(w/v)*(Ns-srr)],[],2); % Calcualte recieve capacity for the right lane of the right side road
    ...
    Stopc = min([Qc,topc],[],2); % Calculate send capacity for cell between the main lanes top
    Rtopc = min([Qc,(w/v)*(Nc-topc)],[],2); % Calculate recieve capacity for cell between the main lanes top
    Sbotc = min([Qc,botc],[],2); % Calculate send capacity for cell between the main lanes bottom
    Rbotc = min([Qc,(w/v)*(Nc-botc)],[],2); % Calculate recieve capacity for cell between the main lanes bottom
