...
if nargin<4 | ~isstruct(varargin{1})
    ...
    set(0,'UserData',0); % Value for autoplay, set to 0, so autoplay is off
    ...
    handles.InOut_T1 = cell(simSize); % Pre allocate space for T1
    handles.InOut_T1(:) = {zeros(2,4)}; % Initialize
    handles.InOut_C = cell(simSize); % Pre allocate space for capacity variables
    handles.InOut_C(:) = {handles.InitCap*ones(2,4)}; % Initialize
    handles.InOut_F = cell(simSize); % Pre allocate space for flow variables
    handles.InOut_F(:) = {handles.InitFlow*ones(2,4)}; % Initialize
    handles.SegmentLength = 9; % Static for now, later controlled by user, has to be odd and equal t or greater than 7.
    handles.InputVolume = 'r'; % As with Segment length, will later be controlled by user 'r' randomizes input
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
Simulation_Single_Time_Step(hObject); % Perform a single time step
...
if get(0,'UserData') % Is true when auto simulation is started, disables all other buttons
    ...
    set(handles.pushbutton4,'Enable','off'); % Disbale all other buttons to avoid complications
    ...
    set(handles.pushbutton4,'Enable','on'); % Enable all other buttons
...
set(handles.pushbutton2, 'String', tempstring); % Update string of autoplay button

while get(0,'UserData') % Check whether autoplay is true
...
input = struct('Network',handles.Network,'Size',handles.Size,'segmentLength',handles.SegmentLength,'initCap',handles.InitCap,'initFlow',handles.InitFlow);
Edit_Window(input,handles.Connections,handles.Segments,handles.InOut_T,handles.Capacity,handles.Flow,hObject); % Open edit window
...
CapandFlow(input,handles.Connections,handles.Segments,handles.Capacity,handles.Flow,handles.InOut_T,hObject); % Open edit window
        ...
        switch road_Network(i,j)
            ...
            case 'EX'
                if strcmp(IV,'r')
                    ...
                    segments(i,j) = {{'r' 'r' 'r' 'r'}};
                    inOut_T(i,j) = {[tempIV{1},tempIV{2},tempIV{3},tempIV{4};zeros(1,4)]};
                else
                    segments(i,j) = {{IV IV IV IV}};
    ...
    for j = 1:simSize
        ...
        switch road_Network(i,j)
            ...
            case char(9472) % Straight L-R
                ...
                [mlt1,mrt1] = GUI_Straight_CF(mlt,mrt,mlt_C,mrt_C,mlt_F,mrt_F); % Calculation function
...
heatmap(visualization,'Colormap',handles.cMap,'ColorLimits',[0,handles.InitCap+0.05],'GridVisible','off','CellLabelColor','none'); %Create heatmap of data
...
for i = 1:simSize % Loop over all segments
    ...
    for j = 1:simSize  
        ...
        if strcmp(road_Network(i,j),'EX')
            temp = segments{i,j};
            temp_Segment = zeros(1,4);
            for k = 1:4
                temp_Segment(k) = temp{k};
            end
        else
        ...
        fprintf(fileID, newline);
        fprintf(fileID, '%d ',temp_Cap(:)); % Save cell capacities
        fprintf(fileID, newline);
        fprintf(fileID, '%d ',temp_Flow(:)); % Save cell flow rates
        fprintf(fileID, newline);
...
capacity = cell(handles.Size);
flow = cell(handles.Size);
inOut_T = cell(handles.Size);
inOut_T1 = cell(handles.Size);


temp = fgetl(fileID);
if strcmp(temp,'r') % Retrieve input volume
    handles.InputVolume = 'r';
else
    ...
    for j = 1:handles.Size
        ...
        switch temp_Type
            ...
            case '6988' % Entrance and exit
                ...
                temp = reshape(str2num(fgetl(fileID)),1,4);
                temp2 = cell(1,4);
                for k = 1:4
                    if temp(k) == 114
                        temp2{k} = 'r';
                ...
                segments(i,j) = {{temp2{1} temp2{2} temp2{3} temp2{4}}}; % Retrieve segment data
                capacity(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                flow(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
            ...
            case {'9472', '9474'} % Straight road segment
                ...
                capacity(i,j) = {reshape(str2num(fgetl(fileID)),segLength,2)};
                flow(i,j) = {reshape(str2num(fgetl(fileID)),segLength,2)};
            ...
            case {'9488', '9484', '9492', '9496'} % Corner road segment
                ...
                capacity(i,j) = {reshape(str2num(fgetl(fileID)),segLength-2,2)};
                flow(i,j) = {reshape(str2num(fgetl(fileID)),segLength-2,2)};
                inOut_T(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                inOut_T1(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                connections(i,j) = {reshape(str2num(fgetl(fileID)),1,4)};
            case {'9508', '9516', '9500', '9524'} % Intersections
                ...
                capacity(i,j) = {reshape(str2num(fgetl(fileID)),segLength+segLength/2-1/2,2)};
                flow(i,j) = {reshape(str2num(fgetl(fileID)),segLength+segLength/2-1/2,2)};
                inOut_T(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                inOut_T1(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                connections(i,j) = {reshape(str2num(fgetl(fileID)),1,4)};
            case '9711' % Roundabouts
                ...
                capacity(i,j) = {reshape(str2num(fgetl(fileID)),segLength-1,4)};
                flow(i,j) = {reshape(str2num(fgetl(fileID)),segLength-1,4)};
                inOut_T(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                inOut_T1(i,j) = {reshape(str2num(fgetl(fileID)),2,4)};
                connections(i,j) = {reshape(str2num(fgetl(fileID)),1,4)};
