...
splitVeh = SplitFunc(ml(ill),ml2sr);
sr = [splitVeh(1);sr]; % Left and not right (ill not ilr) due  to the direction of travel
ml(ill) = splitVeh(2);
sr_C = [ml_C(ill);sr_C];
sr_F = [ml_F(ill);sr_F];

splitVeh = SplitFunc(mr(ill+1),mr2sr);
t1 = [splitVeh(1);t1;sr(2)];
mr(ill+1) = splitVeh(2);
t1_C = [mr_C(ill+1);t1_C;sr_C(2)];
t1_F = [mr_F(ill+1);t1_F;sr_F(2)];

sl = [sl;ml(ilr)];
splitVeh = SplitFunc(sl(end-2),sl2mr);
t2 = [splitVeh(1);t2;mr(ill)];
sl(end-2) = splitVeh(2);
sl_C = [sl_C;ml_C(ilr)];
...
Rsr = max(min([sr_F,(sr_C-sr)],[],2),0); % Calculate the recieve capacity for each cell right lane of side road

Ssl = min([sl_F,sl],[],2); % Calculate the send capacity for each cell left lane of side road
Rsl = max(min([sl_F,(sl_C-sl)],[],2),0); % Calculate the recieve capacity for each cell left lane of side road
...
St1 = min([t1_F,t1],[],2);
Rt1 = max(min([t1_F,(t1_C-t1)],[],2),0);

yt1 = min([St1(1:end-1),Rt1(2:end)],[],2); % Calculate flow for t1 section
yt1(end) = min([yt1(end),ml_C(ill) - ml(ill) - yml(ill-1)]);

St2 = min([t2_F,t2],[],2);
Rt2 = max(min([t2_F,(t2_C-t2)],[],2),0);

yt2 = min([St2(1:end-1),Rt2(2:end)],[],2); % Calculate flow for t2 section
yt2(end) = max(0,min([yt2(end),ml_C(ilr) - ml(ilr) - yml(ilr-1),mr_C(ill) - mr(ill) - ymr(ill-1)])); % ml_C section is culprit of negative vehicles bug!!
