...
if nargin<4 | ~isstruct(varargin{1})
    ...
    initCap = varargin{1}.initCap;
    initFlow = varargin{1}.initFlow;
    ...
    handles.Flow = flow;
    handles.InitCap = initCap;
    handles.InitFlow = initFlow;
...
switch index
    ...
    case 1 % Left - Right
        ...
        capacity(loci,locj) = {initCap*ones(segLength,2)};
        flow(loci,locj) = {initFlow*ones(segLength,2)};
    case 2 % Top - Bottom
    ...
    case 3 % Left - Bottom
        ...
        inOut_T(loci,locj) = {zeros(2,4)};
        capacity(loci,locj) = {initCap*ones(segLength-2,2)};
        flow(loci,locj) = {initFlow*ones(segLength-2,2)};
    case 4 % Bottom - Right
    ...
    case 7 % Intersection Left
        ...
        inOut_T(loci,locj) = {zeros(2,4)};
        capacity(loci,locj) = {initCap*ones(segLength+segLength/2-1/2,2)};
        flow(loci,locj) = {initFlow*ones(segLength+segLength/2-1/2,2)};
    ...
    case 11 % Roundabout
        ...
        inOut_T(loci,locj) = {zeros(2,4)};
        capacity(loci,locj) = {initCap*ones(segLength-1,4)};
        flow(loci,locj) = {initFlow*ones(segLength-1,4)};
    ...
    case 14 % Input
        ...
        segments(loci,locj) = {0};
        inOut_T(loci,locj) = {zeros(2,4)};
        capacity(loci,locj) = {inf*ones(2,4)};
        flow(loci,locj) = {initFlow*ones(2,4)};
    otherwise
