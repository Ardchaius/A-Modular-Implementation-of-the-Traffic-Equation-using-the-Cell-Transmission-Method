...
if nargin<4 | ~isstruct(varargin{1})
    ...
    road_Network = varargin{1}.Network; % Retrieve handles
    CorF = varargin{1}.CorF;
    Segi = varargin{1}.Segi;
    Segj = varargin{1}.Segj;
    size = varargin{1}.Size;
    ...
    handles.Network = road_Network; % Save variables globally
    ...
    if strcmp(CorF,'Capacity') % Check what variables are being altered
        data = capacity{Segi,Segj};
    elseif strcmp(CorF,'Flow')
        data = flow{Segi,Segj};
    else
        data = segments{Segi,Segj};
    end
    ...
    switch road_Network(Segi,Segj) % Format data into a visual format for ease of editing
    ...
    for i = 1:segLength
        for j = 1:segLength
            ...
            temp_S = vis_Segment(loci,locj);
            if vis_Segment(loci,locj) == 114
                temp_S = 'r';
            end
            uicontrol('Parent',hObject,'Style','edit','String',temp_S,'Units','normalized',...
...
switch road_Network(Segi,Segj)
    ...
    case 'EX'
        temp = [data(1,cr) data(cr,end) data(end,cl) data(cl,1)];
        outData = cell(1,4);
        for k  = 1:4
            if temp(k) == 114
                outData{k} = 'r';
...
CPHandles.InOut_T = handles.InOut_T;
